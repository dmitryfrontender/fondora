				...
				mobileScreen ? (
									...
									<ul>
										{newMessages
											...
											: messagesData.map((item: IMessages) => {
													return <MessageComponent item={item} key={item.id} />;
											  })}
									</ul>
								</div>
							</div>
					...
					<div className='messageSidebar'>
						<div className='sidebarTop'>
							...
							<Link to={'/messages'}>
								<div
									className={`textBtn ${pressMessageBtn ? 'activeBtn' : ''}`}
									onClick={() => {
							...
							<div className='messagesWrapper'>
								<ul>
									{newMessages
										? listMessages()
										: messagesData.map((item: IMessages) => {
												return <MessageComponent item={item} typingState={typingState} typingChatId={typingChatId} key={item.id} />;
										  })}
								</ul>
							</div>
						) : (
