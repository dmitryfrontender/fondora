...
//     const barWidth = 3; // Width of each line
//     const gapWidth = 3; // Width of the gap between lines
...
//       xPositionRef.current = width - shiftAmount; // Adjust the position for the next line
//     }
...
//       ctx.fillStyle = '#BAD5FF'; // Color of the line
//       ctx.fillRect(xPositionRef.current, centerY - barHeight, barWidth, barHeight * 2); // Draw lines from the center
//     }
...
//     analyserRef.current.getByteFrequencyData(dataArrayRef.current);
//     drawWaveform();
//     animationTimeoutRef.current = window.setTimeout(updateWaveform, 200); // Set the update interval
//   }, [drawWaveform]);
...
//       analyserRef.current = audioContextRef.current.createAnalyser();
//       source.connect(analyserRef.current);
//       analyserRef.current.fftSize = 256; // Smaller size for more detailed amplitude
//       const bufferLength = analyserRef.current.frequencyBinCount;
//       dataArrayRef.current = new Uint8Array(bufferLength);
//     }
...
//                 :
//                 <>
//                     <span>Нажите кнопку для записи</span>
//                 </>
//             }
...
//         {isRecording ? 
//             // <div className="recordBtn activeRecord">
...
//       {/* <button onClick={handleSaveRecording} disabled={isRecording}>Save</button> */}
//       {/* {audioUrl && (
//         <AudioPlayer audioUrl={audioUrl} />
//       )} */}
//     </div>
...
//     // Shift the canvas content to the left if it overflows
//     if (xPositionRef.current + barWidth >= width) {
//       const imageData = ctx.getImageData(barWidth + gapWidth, 0, width - barWidth - gapWidth, height);
//       ctx.putImageData(imageData, 0, 0);
//       ctx.clearRect(width - barWidth - gapWidth, 0, barWidth + gapWidth, height); // Clear the newly exposed area
//       xPositionRef.current = width - barWidth - gapWidth; // Adjust the position for the next line
//     }
